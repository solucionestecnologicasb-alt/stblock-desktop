...
const ProjectSaverHOC = function (WrappedComponent) {
    class ProjectSaverComponent extends React.Component {
        ...
        collectAiData () {
            try {
                const aiData = {};
                const training = localStorage.getItem('ai_training_data');
                if (training) aiData.training = training;
                const session = localStorage.getItem('ai_session');
                if (session) aiData.session = session;
                const provider = localStorage.getItem('ai_provider');
                const model = provider ? localStorage.getItem(provider + '_model') : null;
                const trainingEnabled = localStorage.getItem('ai_training_enabled');
                const conversation = localStorage.getItem('ai_conversation');
                if (provider || model || trainingEnabled) {
                    aiData.settings = {};
                    if (provider) aiData.settings.provider = provider;
                    if (model) aiData.settings.model = model;
                    if (trainingEnabled) aiData.settings.trainingEnabled = trainingEnabled;
                }
                if (conversation) aiData.conversation = conversation;
        ...
        collectDeviceState () {
            const deviceState = this.props.deviceModeState;
            if (!deviceState) return null;
            const deviceData = {};
            // Device projects
            if (deviceState.deviceProjects && Object.keys(deviceState.deviceProjects).length > 0) {
                deviceData.deviceProjects = deviceState.deviceProjects;
            }
            // Code editor state
            deviceData.codeState = {
                codeViewContent: deviceState.codeViewContent || '',
                isCodeLocked: deviceState.isCodeLocked !== undefined ? deviceState.isCodeLocked : true,
                manualCode: deviceState.manualCode || ''
            };
            // Terminal history
            if (deviceState.terminalOutput && deviceState.terminalOutput.length > 0) {
                deviceData.terminalHistory = deviceState.terminalOutput;
            }
            // Connection state (device, port, terminal settings)
            deviceData.connectionState = {
                selectedDevice: deviceState.selectedDevice ? {
                    deviceId: deviceState.selectedDevice.deviceId,
                    id: deviceState.selectedDevice.id,
                    name: deviceState.selectedDevice.name
                } : null,
                programMode: deviceState.programMode || null,
                selectedPort: deviceState.selectedPort || null,
                terminalSettings: deviceState.terminalSettings || null
            };
            return deviceData;
        }
        ...
        localSnapshot () {
            try {
                ...
                var aiData = this.collectAiData();
                var deviceData = this.collectDeviceState();
                var circuitData = this.props.circuitData || null;
                vm.saveProjectFlynt(aiData, deviceData, circuitData).then(function (blob) {
    ...
    ProjectSaverComponent.propTypes = {
        ...
        deviceModeState: PropTypes.object,
    ...
    const mapStateToProps = (state, ownProps) => {
        ...
        return {
            ...
            deviceModeState: getDevicePersistState(state),
