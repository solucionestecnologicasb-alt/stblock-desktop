...
import {setRunningState, setTurboState, setStartedState, setDebugActive, setDebugArmed} from '../reducers/vm-status';
import {setDebugLine, clearDebugLine} from '../reducers/debug-highlight';
...
const vmListenerHOC = function (WrappedComponent) {
    class VMListener extends React.Component {
        ...
        handleKeyDown (e) {
            ...
            // Don't capture keys from text inputs, textareas, or contenteditable elements
            const tagName = e.target.tagName;
            if (tagName === 'INPUT' || tagName === 'TEXTAREA' || e.target.isContentEditable) return;

            // Don't capture keys from Python editor or other code editors
            if (e.target.classList && (
                e.target.classList.contains('python-editor-textarea') ||
                e.target.classList.contains('no-vm-keyboard')
            )) return;
        ...
        render () {
            const {
                ...
                onDebugLineHighlight,
                onDebugModeDeactivated,
    ...
    const mapDispatchToProps = dispatch => ({
        ...
        onDebugLineHighlight: data => {
            // Usar blockLineMap para convertir blockId → línea Python
            const lineMap = data.blockLineMap || {};
            const pythonLine = lineMap[data.blockId] || null;
            dispatch(setDebugLine({
        ...
        onDebugModeDeactivated: () => {
            dispatch(clearDebugLine());
            dispatch(setDebugActive(false));
            dispatch(setDebugArmed(false));
        }
    });
    return connect(
        mapStateToProps,
