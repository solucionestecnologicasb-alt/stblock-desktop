function Robot() {
  ...
  this.defaultOptions = {
    ...
    wheelMass: 200,
    wheelFriction: 10,
    wheelMaxAcceleration: 20,
    wheelStopActionHoldForce: 30000,
    wheelTireDownwardsForce: -4000,
    caster: true,
    casterMass: 0,
    casterFriction: 0,
    components: []
  ...
  this.load = function (scene, robotStart) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (options.imageType === 'customModel' && options.imageURL) {
        body.visibility = 0;
        var scale = options.modelScale || 1.0;
        BABYLON.SceneLoader.ImportMesh("", "", options.imageURL, scene, function(meshes) {
          meshes.forEach(function(mesh) {
            ...
            if (options.color) {
              mesh.material = babylon.getMaterial(scene, options.color);
            }
          });
        });
      }
      ...
      body.physicsImpostor.registerBeforePhysicsStep(function(){
        var leftStopped = !self.leftWheel || self.leftWheel.mode === self.leftWheel.modes.STOP;
        var rightStopped = !self.rightWheel || self.rightWheel.mode === self.rightWheel.modes.STOP;
        var motorsStopped = leftStopped && rightStopped;
        if (motorsStopped && body.physicsImpostor.getLinearVelocity().lengthSquared() < 0.1) {
      ...
      if (options.wheelsConfig && options.wheelsConfig.length >= 2) {
        let leftW = options.wheelsConfig[0];
        let rightW = options.wheelsConfig[1];

        let leftOpts = Object.assign({}, driveWheelOptions, {
          diameter: leftW.radius * 2,
          width: leftW.width
        });
        ...
        let rightOpts = Object.assign({}, driveWheelOptions, {
          diameter: rightW.radius * 2,
          width: rightW.width
        });
  ...
  this.loadComponents = function(componentsConfig, components, parent) {
    ...
    componentsConfig.forEach(function(componentConfig){
      ...
      } else if (componentConfig.type == 'LineFollowerSensor') {
        component = new LineFollowerSensor(
          self.scene,
          parent,
          componentConfig.position,
          componentConfig.rotation,
          componentConfig.port || ('in' + (++self.sensorCount)),
          componentConfig.options);
      } else if (componentConfig.type == 'GasSensor') {
        component = new GasSensor(
          self.scene,
          parent,
          componentConfig.position,
          componentConfig.rotation,
          componentConfig.port || ('in' + (++self.sensorCount)),
          componentConfig.options);
      } else if (componentConfig.type == 'TemperatureSensor') {
        component = new TemperatureSensor(
          self.scene,
          parent,
          componentConfig.position,
          componentConfig.rotation,
          componentConfig.port || ('in' + (++self.sensorCount)),
          componentConfig.options);
      } else if (componentConfig.type == 'HumiditySensor') {
        component = new HumiditySensor(
          self.scene,
          parent,
          componentConfig.position,
          componentConfig.rotation,
