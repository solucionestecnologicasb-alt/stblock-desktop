var World_Base = function() {
      ...
      function setStartPosRot() {
        if (self.processedOptions.arenaStartPosXYZ instanceof Array) {
          for (let i=0; i < self.processedOptions.arenaStartPosXYZ.length; i++) {
            if (!self.arenaStart[i]) self.arenaStart[i] = {position: new BABYLON.Vector3(0, 0, 0), rotation: new BABYLON.Vector3(0, 0, 0)};
  ...
  this.addModel = async function(scene, options) {
    ...
    var modelURL = String(options.modelURL || "");
    var modelSlash = modelURL.lastIndexOf("/");
    var modelRootURL = modelSlash >= 0 ? modelURL.slice(0, modelSlash + 1) : "";
    var modelFilename = modelSlash >= 0 ? modelURL.slice(modelSlash + 1) : modelURL;
    console.info("[STBLOCK-MODEL-DEBUG] request", {url: modelURL, rootUrl: modelRootURL, filename: modelFilename, scale: options.modelScale});
    var results;
    try {
      results = await BABYLON.SceneLoader.ImportMeshAsync(null, modelRootURL, modelFilename, scene);
    } catch (err) {
      console.error("[STBLOCK-MODEL-DEBUG] import-error", {url: modelURL, message: err && err.message, error: err});
      throw err;
    }
    console.info("[STBLOCK-MODEL-DEBUG] imported", {url: modelURL, meshes: results.meshes.map(function(item) {
      return {name: item.name, vertices: typeof item.getTotalVertices === "function" ? item.getTotalVertices() : 0};
    })});
    ...
    // Get bounding box
    let firstMesh = meshes.find(function(mesh) {
      return typeof mesh.getBoundingInfo == 'function' &&
        typeof mesh.getTotalVertices == 'function' && mesh.getTotalVertices() > 0;
    });
    if (!firstMesh) throw new Error('Model has no renderable meshes');
    let min = firstMesh.getBoundingInfo().boundingBox.minimumWorld;
    let max = firstMesh.getBoundingInfo().boundingBox.maximumWorld;

    for (let i=0; i<meshes.length; i++) {
      if (typeof meshes[i].getBoundingInfo != 'function') continue;
    ...
    console.info("[STBLOCK-MODEL-DEBUG] bounds", {url: modelURL, scale: options.modelScale, size: [bx, by, bz], center: bounding.boundingBox.center.asArray()});
    ...
    var proxyMaterial = new BABYLON.StandardMaterial(id + '_proxyMaterial', scene);
    proxyMaterial.alpha = 0;
    proxyMaterial.disableDepthWrite = true;
    mesh.material = proxyMaterial;
    options.position.y = Math.max(options.position.y, by / 2);

    mesh.position = options.position;
    mesh.rotation = options.rotation;

    // Keep every imported root visible and move the complete model with its physics proxy.
    var modelRoot = new BABYLON.TransformNode(id + '_visualRoot', scene);
    modelRoot.parent = mesh;
    modelRoot.scaling = new BABYLON.Vector3(options.modelScale, options.modelScale, options.modelScale);
    let offset = bounding.boundingBox.center.scale(options.modelScale);
    modelRoot.position = offset.scale(-1);
    var importedRoots = meshes.filter(function(importedMesh) { return !importedMesh.parent; });
    importedRoots.forEach(function(importedMesh) { importedMesh.parent = modelRoot; });
    meshes.forEach(function(importedMesh) {
      importedMesh.isVisible = true;
      importedMesh.visibility = 1;
      if (!importedMesh.material && typeof importedMesh.getTotalVertices === "function" && importedMesh.getTotalVertices() > 0) {
        importedMesh.material = options.material;
      }
    });
