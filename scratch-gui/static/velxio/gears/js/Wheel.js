function Wheel(scene, parent, pos, rot, port, options) {
  ...
  this.loadImpostor = function(){
    ...
    self.mesh.physicsImpostor.registerBeforePhysicsStep(function(){
      var motorStopped = self.mode === self.modes.STOP;
      if (motorStopped && self.mesh.physicsImpostor.getLinearVelocity().lengthSquared() < 0.1) {
  ...
  this.setMotorSpeed = function(delta, reversed=false) {
    ...
    // Apply drift if it is the left wheel and drift is enabled
    if (self.parent && self.parent.component && self.parent.component.options) {
      var robotOpts = self.parent.component.options;
      var driftEnabled = robotOpts.chassis && robotOpts.chassis.driftEnabled;
      var driftLeft = robotOpts.chassis && robotOpts.chassis.driftLeft;
      if (driftEnabled && typeof driftLeft !== 'undefined') {
        var isLeftWheel = (self.parent.component.leftWheel === self);
        if (isLeftWheel) {
          speed = speed * (1 - driftLeft / 100);
        }
      }
    }
