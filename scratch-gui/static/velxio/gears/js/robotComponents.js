...
function ArmActuator(scene, parent, pos, rot, port, options) {
  ...
  this.init = function() {
    ...
    self.setEquippedModel = function(index) {
      if (!self.options.models || !self.options.models[index]) return;
      var model = self.options.models[index];
      if (self.loadedCustomMeshes) {
        self.loadedCustomMeshes.forEach(function(m) { m.dispose(); });
      }
      self.loadedCustomMeshes = [];
      arm.visibility = 0;
      BABYLON.SceneLoader.ImportMesh("", "", model.url, scene, function(meshes) {
        meshes.forEach(function(mesh) {
          mesh.parent = arm;
          mesh.scaling = new BABYLON.Vector3(model.scale || 1.0, model.scale || 1.0, model.scale || 1.0);
          mesh.material = armMat;
          if (scene.shadowGenerator) {
            scene.shadowGenerator.addShadowCaster(mesh);
          }
          self.loadedCustomMeshes.push(mesh);
        });
      });
    };

    self.loadedCustomMeshes = [];
    if (self.options.models && self.options.models.length > 0) {
      self.setEquippedModel(0);
    }
  };
...
// Helpers for new custom sensors
function isInsideAnyZone(sensorPos) {
  if (!babylon || !babylon.world || !babylon.world.options || !babylon.world.options.objects) return false;
  var objects = babylon.world.options.objects;
  var scene = babylon.scene;
  for (var i = 0; i < objects.length; i++) {
    var obj = objects[i];
    if (obj.editorType === 'zone') {
      var mesh = scene.getMeshByName(obj.id) || scene.meshes.find(function(m) { return m.stblockObjectId === obj.id; });
      if (mesh && mesh.intersectsPoint(sensorPos)) {
        return true;
      }
    }
  }
...
function getLightIntensityAt(sensorPos) {
  if (!babylon || !babylon.world || !babylon.world.options || !babylon.world.options.objects) return 0;
  var objects = babylon.world.options.objects;
  var scene = babylon.scene;
  var maxIntensity = 0;
  for (var i = 0; i < objects.length; i++) {
    var obj = objects[i];
    if (obj.editorType === 'light') {
      var mesh = scene.getMeshByName(obj.id) || scene.meshes.find(function(m) { return m.stblockObjectId === obj.id; });
      if (mesh) {
        var dist = BABYLON.Vector3.Distance(sensorPos, mesh.getAbsolutePosition());
        var range = obj.lightRange || 50;
        if (dist < range) {
          var intensity = (obj.lightIntensity || 1) * (1 - dist / range);
...
function getGroundSurfaceType() {
  if (!babylon || !babylon.world || !babylon.world.options) return 'smooth';
  return babylon.world.options.surfaceType || 'smooth';
}

// Line Follower Sensor (3 channel)
function LineFollowerSensor(scene, parent, pos, rot, port, options) {
  var self = this;
  this.type = 'LineFollowerSensor';
  this.port = port;
  ...
  this.init = function() {
    var bodyMat = babylon.getMaterial(scene, '222222');
    var body = BABYLON.MeshBuilder.CreateBox('lineFollowerBody', {width: 4, height: 0.6, depth: 1.5}, scene);
    self.body = body;
    body.component = self;
    body.material = bodyMat;
    body.position = self.position;
    ...
    self.leftSensor = new ColorSensor(scene, body, [-1.5, -0.3, 0], [Math.PI / 2, 0, 0], port + '_L', subOptions);
    self.centerSensor = new ColorSensor(scene, body, [0, -0.3, 0], [Math.PI / 2, 0, 0], port + '_C', subOptions);
    self.rightSensor = new ColorSensor(scene, body, [1.5, -0.3, 0], [Math.PI / 2, 0, 0], port + '_R', subOptions);
  };

  this.render = function(delta) {
    self.leftSensor.render(delta);
    self.centerSensor.render(delta);
    self.rightSensor.render(delta);
  };

  this.getReadings = function() {
    var lRGB = self.leftSensor.getRGB();
    var cRGB = self.centerSensor.getRGB();
    var rRGB = self.rightSensor.getRGB();
    
    var lRef = (lRGB[0] + lRGB[1] + lRGB[2]) / 3 / 255;
    var cRef = (cRGB[0] + cRGB[1] + cRGB[2]) / 3 / 255;
    var rRef = (rRGB[0] + rRGB[1] + rRGB[2]) / 3 / 255;

    return [lRef, cRef, rRef];
  };

  this.init();
}

// Gas Sensor (returns ppm)
function GasSensor(scene, parent, pos, rot, port, options) {
  var self = this;
  this.type = 'GasSensor';
  this.port = port;
  this.options = options;
  ...
  this.init = function() {
    var bodyMat = babylon.getMaterial(scene, '888888');
    var body = BABYLON.MeshBuilder.CreateCylinder('gasSensorBody', {diameter: 1.5, height: 1.2}, scene);
    self.body = body;
    body.component = self;
    body.material = bodyMat;
    body.position = self.position;
  ...
  this.getPPM = function() {
    var basePPM = 350 + Math.random() * 20;
    var pos = self.body.getAbsolutePosition();
    if (isInsideAnyZone(pos)) {
      basePPM += 1500 + Math.random() * 300;
    }
    return basePPM;
  };
...
// Temperature Sensor (returns Celsius)
function TemperatureSensor(scene, parent, pos, rot, port, options) {
  var self = this;
  ...
  this.init = function() {
    var bodyMat = babylon.getMaterial(scene, 'C0C0C0');
    var body = BABYLON.MeshBuilder.CreateCylinder('tempSensorBody', {diameter: 0.4, height: 2.2}, scene);
    self.body = body;
    body.component = self;
    body.material = bodyMat;
    body.position = self.position;
  ...
  this.getTemperature = function() {
    var baseTemp = 22.0 + Math.random() * 0.5;
    var pos = self.body.getAbsolutePosition();
    if (isInsideAnyZone(pos)) {
      baseTemp += 15.0;
    }
    var lightIntensity = getLightIntensityAt(pos);
    baseTemp += lightIntensity * 45.0;
    return baseTemp;
  };
...
// Humidity Sensor (returns percentage)
function HumiditySensor(scene, parent, pos, rot, port, options) {
  var self = this;
  ...
  this.init = function() {
    var bodyMat = babylon.getMaterial(scene, '0077CC');
    var body = BABYLON.MeshBuilder.CreateBox('humiditySensorBody', {width: 1.4, height: 0.3, depth: 1.4}, scene);
    self.body = body;
    body.component = self;
    body.material = bodyMat;
    body.position = self.position;
  ...
  this.getHumidity = function() {
    var surface = getGroundSurfaceType();
    var baseHum = 45.0 + Math.random() * 2;
    if (surface === 'water') {
      baseHum = 95.0 + Math.random() * 5;
    } else if (surface === 'earth' || surface === 'grass') {
      baseHum = 70.0 + Math.random() * 5;
    } else if (surface === 'sand') {
      baseHum = 25.0 + Math.random() * 3;
    }
    var pos = self.body.getAbsolutePosition();
    if (isInsideAnyZone(pos)) {
      baseHum = Math.min(100.0, baseHum + 20.0);
    }
    return Math.min(100.0, Math.max(0.0, baseHum));
  };
