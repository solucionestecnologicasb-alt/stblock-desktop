...
var $builtinmodule = function(name) {
  ...
  mod.Motor = Sk.misceval.buildClass(mod, function($gbl, $loc) {
    ...
    $loc.__init__ = new Sk.builtin.func(function(self, address) {
      console.log('[Motor.__init__] address:', address.v);
      console.log('[Motor.__init__] robot.processedOptions.wheels:', robot.processedOptions ? robot.processedOptions.wheels : 'undefined');
      console.log('[Motor.__init__] robot.leftWheel:', robot.leftWheel ? 'EXISTS' : 'NULL');
      console.log('[Motor.__init__] robot.rightWheel:', robot.rightWheel ? 'EXISTS' : 'NULL');

      if (robot.processedOptions.wheels && address.v == 'outA') {
        self.motor = robot.leftWheel;
        console.log('[Motor.__init__] Asignado leftWheel');
      } else if (robot.processedOptions.wheels && address.v == 'outB') {
        self.motor = robot.rightWheel;
        console.log('[Motor.__init__] Asignado rightWheel');
      } else {
        self.motor = robot.getComponentByPort(address.v);
        console.log('[Motor.__init__] Buscando por puerto:', address.v, '-> resultado:', self.motor ? 'FOUND' : 'NOT FOUND');
        if (!self.motor) {
