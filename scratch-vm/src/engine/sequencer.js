...
class Sequencer {
    ...
    stepThread (thread) {
        ...
        while ((currentBlockId = thread.peekStack())) {
            ...
            // In debug mode, glow each block immediately and yield after one block
            if (this.runtime.debugMode) {
                // Clear previous block glow before lighting the new one
                if (thread._prevDebugBlockId) {
                    this.runtime.glowBlock(thread._prevDebugBlockId, false);
                }
                this.runtime.glowBlock(currentBlockId, true);
                thread._prevDebugBlockId = currentBlockId;
                this.runtime.emit('DEBUG_LINE_HIGHLIGHT', {
                    blockId: currentBlockId,
                    targetId: thread.target ? thread.target.id : null,
                    blockLineMap: this.runtime.blockLineMap || {}
                });
                // Advance to next block before yielding (mirrors block advancement below)
