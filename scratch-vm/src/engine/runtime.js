...
const defaultBlockPackages = {
    ...
    scratch3_game: require('../blocks/scratch3_game'),
    scratch3_logic: require('../blocks/scratch3_logic'),
...
class Runtime extends EventEmitter {
    constructor () {
        ...
        /**
         * STBlock device profile persisted with the project.
         * Hardware services consume this metadata without coupling the VM to Tauri.
         * @type {?object}
         */
        ...
        /**
         * Whether debug/step-through mode is active.
         * @type {boolean}
         */
        this.debugMode = false;

        /**
         * Delay between debug steps in ms.
         * @type {number}
         */
        this.debugSpeed = 100;

        /**
         * Reference to the debug stepping timeout.
         * @type {?number}
         */
        this._debugTimeout = null;

        /**
         * Flag used by _step() to distinguish calls from _debugStep() vs the
         * normal stepping interval when debugMode is active.
         * @type {boolean}
         */
        this._debugStepping = false;

        /**
         * Map of blockId -> pythonLineNumber for the current project.
         * @type {object}
         */
        this.blockLineMap = {};
    ...
    /**
     * Event name for debug line highlight.
     * @const {string}
     */
    static get DEBUG_LINE_HIGHLIGHT () {
        return 'DEBUG_LINE_HIGHLIGHT';
    }

    /**
     * Event name for debug mode activated.
     * @const {string}
     */
    static get DEBUG_MODE_ACTIVATED () {
        return 'DEBUG_MODE_ACTIVATED';
    }

    /**
     * Event name for debug mode deactivated.
     * @const {string}
     */
    static get DEBUG_MODE_DEACTIVATED () {
        return 'DEBUG_MODE_DEACTIVATED';
    }
    ...
    /**
     * Event name for code generation update (device mode).
     * @const {string}
     */
    static get CODE_GENERATED () {
        return 'CODE_GENERATED';
    }

    /**
     * Event name to request code regeneration.
     * @const {string}
     */
    ...
    /**
     * Event name for serial data received from device.
     * @const {string}
     */
    static get SERIAL_DATA_RECEIVED () {
    ...
    setDeviceProfile (device, programMode = null, emitProjectChanged = true) {
        ...
        if (this._deviceProfile && this._deviceProfile.deviceId === 'stbBoardV2') {
            const puertosCat = this._deviceBlockInfo.find(c => c.id === 'arduino_stbv2puertos');
            if (puertosCat) {
                // --- Bloques de Ultrasónico (solo reporteros básicos) ---
                const portMenu = {
                    type: 'field_dropdown',
                    name: 'PORT',
                    options: [
                        ['puerto 2', '2'], ['puerto 3', '3'], ['puerto 4', '4'],
                        ['puerto 5', '5'], ['puerto 7', '7'], ['puerto 8', '8'],
                        ['puerto 9', '9'], ['puerto 10', '10']
                    ]
                };
                const ultraBlocks = [
                    {
                        info: {blockType: 'reporter', opcode: 'readDistCm',
                            text: 'distancia ultrasonido [PORT] cm',
                            arguments: {PORT: {type: 'string', menu: 'STBV2_PORT_MENU'}}},
                        json: {type: 'arduino_stbv2puertos_readDistCm', inputsInline: true,
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [portMenu], message0: 'distancia ultrasonido %1 cm'},
                        xml: '<block type="arduino_stbv2puertos_readDistCm"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readDistInch',
                            text: 'distancia ultrasonido [PORT] in',
                            arguments: {PORT: {type: 'string', menu: 'STBV2_PORT_MENU'}}},
                        json: {type: 'arduino_stbv2puertos_readDistInch', inputsInline: true,
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [portMenu], message0: 'distancia ultrasonido %1 in'},
                        xml: '<block type="arduino_stbv2puertos_readDistInch"></block>'
                    }
                ];
                puertosCat.blocks.push(...ultraBlocks);

                // --- Bloques de sensor de color TCS34725 (solo reporteros y booleano genérico) ---
                const colorMenu = {
                    type: 'field_dropdown',
                    name: 'COLOR',
                    options: [
                        ...
                        ['amarillo', 'amarillo'], ['marron', 'marron']
                    ]
                };
                const colorBlocks = [
                    {
                        info: {blockType: 'reporter', opcode: 'readColorName',
                            text: 'color detectado',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readColorName',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'String', outputShape: 1,
                            args0: [], message0: 'color detectado'},
                        xml: '<block type="arduino_stbv2puertos_readColorName"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readRed',
                            text: 'rojo',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readRed',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [], message0: 'rojo'},
                        xml: '<block type="arduino_stbv2puertos_readRed"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readGreen',
                            text: 'verde',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readGreen',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [], message0: 'verde'},
                        xml: '<block type="arduino_stbv2puertos_readGreen"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readBlue',
                            text: 'azul',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readBlue',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [], message0: 'azul'},
                        xml: '<block type="arduino_stbv2puertos_readBlue"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readClear',
                            text: 'luz',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readClear',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [], message0: 'luz'},
                        xml: '<block type="arduino_stbv2puertos_readClear"></block>'
                    },
                    {
                        info: {blockType: 'reporter', opcode: 'readHue',
                            text: 'matiz',
                            arguments: {}},
                        json: {type: 'arduino_stbv2puertos_readHue',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Number', outputShape: 1,
                            args0: [], message0: 'matiz'},
                        xml: '<block type="arduino_stbv2puertos_readHue"></block>'
                    },
                    {
                        info: {blockType: 'boolean', opcode: 'isColor',
                            text: 'es color [COLOR]?',
                            arguments: {COLOR: {type: 'string', menu: 'STBV2_COLOR_MENU'}}},
                        json: {type: 'arduino_stbv2puertos_isColor',
                            category: 'Puertos', colour: '#4C97FF', colourSecondary: '#4280D7',
                            colourTertiary: '#3373CC', output: 'Boolean', outputShape: 2,
                            args0: [colorMenu], message0: 'es color %1?'},
                        xml: '<block type="arduino_stbv2puertos_isColor"></block>'
                    }
                ];
                puertosCat.blocks.push(...colorBlocks);

                // --- Agregar bloques seriales a la categoría Serie existente ---
                const serieCat = this._deviceBlockInfo.find(c => c.id === 'arduino_serial');
                if (serieCat) {
                    const serialBlocks = [
                        {
                            info: {blockType: 'command', opcode: 'serialPrint',
                                text: 'imprimir en serial [TEXTO]',
                                arguments: {TEXTO: {type: 'string', defaultValue: 'Hola'}}},
                            json: {type: 'arduino_serial_serialPrint', inputsInline: true,
                                category: 'Serie', colour: '#9966FF', colourSecondary: '#855CD6',
                                colourTertiary: '#774DCB', outputShape: 3,
                                previousStatement: null, nextStatement: null,
                                args0: [{type: 'input_value', name: 'TEXTO'}],
                                message0: 'imprimir en serial %1'},
                            xml: '<block type="arduino_serial_serialPrint"><value name="TEXTO"><shadow type="text"><field name="TEXT">Hola</field></shadow></value></block>'
                        },
                        {
                            info: {blockType: 'command', opcode: 'serialPrintln',
                                text: 'imprimir linea en serial [TEXTO]',
                                arguments: {TEXTO: {type: 'string', defaultValue: 'Hola'}}},
                            json: {type: 'arduino_serial_serialPrintln', inputsInline: true,
                                category: 'Serie', colour: '#9966FF', colourSecondary: '#855CD6',
                                colourTertiary: '#774DCB', outputShape: 3,
                                previousStatement: null, nextStatement: null,
                                args0: [{type: 'input_value', name: 'TEXTO'}],
                                message0: 'imprimir linea en serial %1'},
                            xml: '<block type="arduino_serial_serialPrintln"><value name="TEXTO"><shadow type="text"><field name="TEXT">Hola</field></shadow></value></block>'
                        }
                    ];
                    serieCat.blocks.push(...serialBlocks);
                }
            }
        }
    ...
    setHardwareModeActive (active) {
        this._hardwareModeActive = Boolean(active);
    }

    isHardwareModeActive () {
        return this._hardwareModeActive;
    }

    getDeviceProfile () {
        return this._deviceProfile ? JSON.parse(JSON.stringify(this._deviceProfile)) : null;
    }
     ...
     */
    getBlocksJSON () {
        return this._blockInfo.concat(this._deviceBlockInfo).reduce(
            (result, categoryInfo) =>
                result.concat(
    ...
    /**
     * Set the baudrate of the connected peripheral.
     * @param {string} extensionId - the id of the extension.
     * @param {number} baudrate - the baudrate.
     */
    setPeripheralBaudrate (extensionId, baudrate) {
        if (this.peripheralExtensions[extensionId]) {
            if (typeof this.peripheralExtensions[extensionId].setBaudrate === 'function') {
                this.peripheralExtensions[extensionId].setBaudrate(baudrate);
            }
        }
    }

    /**
     * Update the board type of a registered peripheral (e.g., arduinoUno → stbBoardV2).
     * Called when the user selects a different device in the UI.
     * @param {string} extensionId - the id of the extension.
     * @param {string} boardType - the new board type ID.
     */
    setPeripheralBoardType (extensionId, boardType) {
        if (this.peripheralExtensions[extensionId]) {
            if (typeof this.peripheralExtensions[extensionId].setBoardType === 'function') {
                this.peripheralExtensions[extensionId].setBoardType(boardType);
            }
        }
    }
    ...
    stopAll () {
        ...
        // Desactivar debug mode al detener
        if (this.debugMode) {
    ...
    /**
     * Activate or deactivate debug/step-through mode.
     * The normal stepping interval continues running but _step() uses the
     * _debugStepping flag to skip execution unless called from _debugStep().
     * @param {boolean} active True to activate debug mode.
     * @param {number} [speed] Delay between debug steps in ms.
     */
    setDebugMode (active, speed) {
        this.debugMode = active;
        if (active) {
            this.debugSpeed = typeof speed === 'number' ? speed : 100;
            // Clear any pending script glows from previous non-debug runs
            for (let i = 0; i < this._scriptGlowsPreviousFrame.length; i++) {
                this.glowScript(this._scriptGlowsPreviousFrame[i], false);
            }
            this._scriptGlowsPreviousFrame = [];
            this.emit(Runtime.DEBUG_MODE_ACTIVATED, {speed: this.debugSpeed});
        } else {
            if (this._debugTimeout) {
                clearTimeout(this._debugTimeout);
                this._debugTimeout = null;
            }
            this.emit(Runtime.DEBUG_MODE_DEACTIVATED);
        }
    }

    /**
     * Execute a single debug step: runs one tick of _step() and
     * schedules the next tick after debugSpeed ms.
     * Only call this when debugMode is true.
     */
    _debugStep () {
        if (!this.debugMode) {
            return;
        ...
        this._debugStepping = true;
        this._step();
        this._debugStepping = false;

        // Check if there are still active threads
        if (this.debugMode && this.threads.length > 0) {
            this._debugTimeout = setTimeout(() => {
                this._debugStep();
            }, this.debugSpeed);
        } else if (this.threads.length === 0) {
            // All threads finished - exit debug mode
            this.setDebugMode(false);
        }
    }

    /**
     * Start execution in debug/step-through mode.
     * Call this AFTER greenFlag() has been called (threads are already started).
     */
    startDebug () {
     ...
     */
    _step () {
        // En debug mode, solo _debugStep() puede ejecutar _step()
        if (this.debugMode && !this._debugStepping) {
            return;
        }
        ...
        // In debug mode, skip edge-activated hats and glow updates
        // to avoid re-triggering scripts and double illumination
        if (!this.debugMode) {
        ...
        // En debug mode el glow se maneja individualmente desde el sequencer
        if (!this.debugMode) {
