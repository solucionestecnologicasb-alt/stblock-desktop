...
class VirtualMachine extends EventEmitter {
    constructor () {
        ...
        this.runtime.on(Runtime.DEBUG_LINE_HIGHLIGHT, data => {
            this.emit(Runtime.DEBUG_LINE_HIGHLIGHT, data);
        });
        this.runtime.on(Runtime.DEBUG_MODE_ACTIVATED, data => {
            this.emit(Runtime.DEBUG_MODE_ACTIVATED, data);
        });
        this.runtime.on(Runtime.DEBUG_MODE_DEACTIVATED, data => {
            this.emit(Runtime.DEBUG_MODE_DEACTIVATED, data);
        });
        ...
        this.runtime.on(Runtime.CODE_GENERATED, code => {
            this.emit(Runtime.CODE_GENERATED, code);
        });
        this.runtime.on(Runtime.SERIAL_DATA_RECEIVED, data => {
            this.emit(Runtime.SERIAL_DATA_RECEIVED, data);
        });
        this.runtime.on('PERIPHERAL_UPLOAD_STATUS', data => {
            this.emit('PERIPHERAL_UPLOAD_STATUS', data);
        });
    ...
    /**
     * Activate debug/step-through mode.
     * @param {number} speed Delay between debug steps in ms.
     */
    setDebugMode (speed) {
        this.runtime.setDebugMode(true, speed);
    }

    /**
     * Disable debug/step-through mode.
     */
    disableDebugMode () {
        this.runtime.setDebugMode(false);
    }

    /**
     * Start execution in debug mode. Call after greenFlag().
     */
    startDebug () {
        this.runtime.startDebug();
    }
    ...
    setDeviceProfile (device, programMode = null) {
        this.runtime.setDeviceProfile(device, programMode);
        if (device && device.generator === 'arduino' && !this.extensionManager.isExtensionLoaded('arduino')) {
            this.extensionManager.loadExtensionURL('arduino').catch(error => {
                log.error(`Unable to load Arduino extension: ${error}`);
            });
        }
    }

    getDeviceProfile () {
        return this.runtime.getDeviceProfile();
    }

    setHardwareModeActive (active) {
        this.runtime.setHardwareModeActive(active);
    ...
    /**
     * Generate Arduino code from current blocks
     * @returns {string} Generated Arduino C++ code
     */
    generateArduinoCode () {
        const ArduinoGenerator = require('./generators/arduino');
        const generator = new ArduinoGenerator();

        const target = this.runtime.getEditingTarget();

        if (!target) {
            return '// No hay bloques para generar\n\nvoid setup() {\n}\n\nvoid loop() {\n}\n';
        }

        // Access blocks - make a shallow copy to avoid reference issues
        let blocks = {};
        if (target.blocks && target.blocks._blocks) {
            // Create a shallow copy of the blocks object
            const sourceBlocks = target.blocks._blocks;

            // Try iterating with for...in
            for (const id in sourceBlocks) {
                if (Object.prototype.hasOwnProperty.call(sourceBlocks, id)) {
                    blocks[id] = sourceBlocks[id];
                }
            }
        }

        // If still no blocks, check all targets
        if (Object.keys(blocks).length === 0) {
            const allTargets = this.runtime.targets;
            for (const t of (allTargets || [])) {
                if (t.blocks && t.blocks._blocks) {
                    const sourceBlocks = t.blocks._blocks;
                    for (const id in sourceBlocks) {
                        if (Object.prototype.hasOwnProperty.call(sourceBlocks, id)) {
                            blocks[id] = sourceBlocks[id];
                        }
        ...
        return generator.generateCode(blocks, this.runtime);
    }

    /**
     * Request code regeneration and emit event
     */
    requestCodeUpdate () {
        const code = this.generateArduinoCode();
        this.runtime.emit(this.runtime.constructor.CODE_GENERATED, code);
        return code;
    }
    ...
    /**
     * Upload realtime firmware to the extension's connected peripheral.
     * @param {string} extensionId - the id of the extension.
     */
    uploadFirmwareToPeripheral (extensionId) {
        this.runtime.uploadFirmwareToPeripheral(extensionId);
    }
    ...
    /**
     * Update the board type of a registered peripheral.
     * @param {string} extensionId - the id of the extension.
     * @param {string} boardType - the new board type ID.
     */
    setPeripheralBoardType (extensionId, boardType) {
        this.runtime.setPeripheralBoardType(extensionId, boardType);
    }
    ...
    /**
     * Save a project as .flynt — wraps .sb3 contents + AI data + device data + circuit data in a single zip.
    ...
    saveProjectFlynt (aiData, deviceData, circuitData, sketchforgeData) {
        ...
        if (aiData) {
            ...
            if (aiData.conversation) {
                zip.file('ai/conversation.json', typeof aiData.conversation === 'string' ?
                    aiData.conversation : JSON.stringify(aiData.conversation, null, 2));
            }
        }

        // Device data - projects per device
        if (deviceData) {
            // Device projects (workspaces per device)
            if (deviceData.deviceProjects && Object.keys(deviceData.deviceProjects).length > 0) {
                zip.file('devices/projects.json', JSON.stringify(deviceData.deviceProjects, null, 2));
            }
            // Code editor state (lock state, manual code)
            if (deviceData.codeState) {
                zip.file('devices/code-state.json', JSON.stringify(deviceData.codeState, null, 2));
            }
            // Terminal history
            if (deviceData.terminalHistory) {
                zip.file('devices/terminal.json', JSON.stringify(deviceData.terminalHistory, null, 2));
            }
            // Connection state (selected device, port, terminal settings)
            if (deviceData.connectionState) {
                zip.file('devices/connection-state.json', JSON.stringify(deviceData.connectionState, null, 2));
            }
        }
    ...
    /**
     * Extract device data from a .flynt zip.
     * @param {ArrayBuffer} buffer The .flynt zip content.
     * @return {!Promise<object|null>} Device data object: {deviceProjects, codeState, terminalHistory, connectionState, circuitData} or null.
     */
    static extractFlyntDeviceData (buffer) {
        return JSZip.loadAsync(buffer).then(function (zip) {
            var deviceData = {};
            var promises = [];
            ...
            function readFile(name, key) {
                ...
                if (file && !file.dir) {
                    promises.push(file.async('string').then(function (text) {
                        try {
                            deviceData[key] = JSON.parse(text);
                        } catch (e) {
                            deviceData[key] = text;
                        }
                    }));
            ...
            readFile('devices/projects.json', 'deviceProjects');
            readFile('devices/code-state.json', 'codeState');
